...
export default function Navbar() {
  ...
  const handlePayment = async () => {
    if (!agreement) {
      toast.warning(
        "Please agree to both the disclaimer and terms & conditions"
      );
  ...
  return (
    <div
      ref={main}
      className={`bg-primary py-3 sticky top-0 z-50 transition-all duration-300 ${
                ...
                <>
                  <div ref={prof} className="relative">
                    <div
                      ...
                      ref={prof}
                      className="cursor-pointer hover:bg-gray-200/20 p-2 transition rounded-full flex items-center gap-2"
                    >
                      <p>Hi, {userInfo?.data.name}</p>
                    </div>
                    <div
                      className={`absolute right-0 top-12 min-w-[160px] bg-white/50 backdrop-blur-2xl text-black shadow-lg rounded-md transition-all duration-300 transform z-50 ${
                        profileOpen
                    ...
                    <div className="flex flex-col p-3 divide-y">
                      {profile?.map((item) => (
                        <Link
                          ...
                          className={`transition-all duration-200 py-2 px-3  hover:bg-gray-100/20  ${
                            activeItem === item.link
          ...
          >
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.link}
                className={`py-3 text-sm w-full text-center transition-all duration-200 ${
                  activeItem === item.link
                    ? "text-secondary font-semibold bg-[#0d3456]"
